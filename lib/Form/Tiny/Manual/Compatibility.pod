...
=head3 Subroutine call parameters are getting standarized
...
Because of this, parameters they get passed tend to vary. For example the C<default> sub will get passed a single parameter, which is a form instance, while form validator will not get passed a form instance, making it hard or impossible to make use of instance data in certain places.

To combat this, in the future release all listed places will consistently get passed the form instance as their first parameter, followed by their current parameters:
...
Starting with I<2.03>, a new import flag C<-consistent> will be added, which enables the new parameter lists:

	use Form::Tiny -consistent;

A warning will be raised if Form::Tiny is imported without this flag. After the deprecation period and removal of old syntax, the flag will be made redundant.
